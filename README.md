...
- **用户管理（仅管理员）**：用户列表（角色 / 创建时间 / 最后登录）、删除用户、重置密码（生成随机临时密码，仅展示一次）、提升 / 降级角色；最后一个管理员不可删不可降
- **三本审计账**：登录记录（登录 / 登录失败 / 登出 / 改密 / 注册 / 重置密码 / 角色变更 / 删除）、访问记录（页面级访问）、操作日志（经过门禁的每一次 API 调用与 WebSocket 连接，含方法 / 路径 / 响应状态 / 来源 IP），均支持按用户、类型、路径、状态过滤
...
3. 管理员：用户表（重置密码 / 角色调整 / 删除）+ 登录记录 / 访问记录 / 操作日志三个审计页
4. 普通用户：个人信息卡 + 修改密码 + 自己的登录记录
5. 数据与审计文件都在 `~/.dsh/user-management/`（`users.json` / `sessions.json` / `activity.jsonl` / `audit.jsonl`，0600 权限，原子写；审计账本滚动保留最近 5000 条）
