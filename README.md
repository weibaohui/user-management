...
- **TOTP 两步验证（RFC 6238，默认关闭，自行开启）**：设置页扫码（二维码）或手输密钥绑定验证器 App，输入一个实时动态码完成绑定；开启后登录 = 用户名 + 密码 + 6 位动态码，密码正确后才会要求验证码；动态码 **30 秒一换、一次一用**（±1 窗口容时钟漂移，用过的码立即作废）；连续错 5 次锁定 60 秒防爆破（内存态，重启清零）；关闭需输登录密码确认，管理员可对任意用户重置。零第三方依赖，兼容 Google Authenticator / 1Password / 微软验证器 / Aegis 等
...
5. **开启两步验证**：设置页「两步验证」页签（或个人信息卡）→ 开启 → 验证器 App 扫二维码（或手动输入密钥）→ 输入 App 显示的 6 位动态码 → 绑定完成；之后登录先输用户名密码，再按提示输动态码（浏览器会记住用户名密码，动态码每次现查 App）
