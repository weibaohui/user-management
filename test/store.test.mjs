...
test('ip bans: validate, dedupe, list, unban, persistence', async () => {
  ...
  await assert.rejects(() => store.banIp('127.0.0.1'), (e) => e.code === 'protected_ip')
  await assert.rejects(() => store.banIp('127.0.0.99'), (e) => e.code === 'protected_ip', 'whole loopback /8 is protected')
  await assert.rejects(() => store.banIp('::1'), (e) => e.code === 'protected_ip')
